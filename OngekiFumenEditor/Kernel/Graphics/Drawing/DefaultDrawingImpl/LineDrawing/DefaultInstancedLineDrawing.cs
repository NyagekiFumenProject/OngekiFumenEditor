...
#pragma warning restore CS0649
        ...
        public void Begin(IDrawingContext target, float lineWidth)
        {
            ...
            /*
#if DEBUG
            GL.ValidateProgram(shader.ShaderProgram);
            GL.GetProgram(shader.ShaderProgram, GetProgramParameterName.ValidateStatus, out int validateStatus);
            if (validateStatus == (int)All.False)
            {
                string infoLog = GL.GetProgramInfoLog(shader.ShaderProgram);
                throw new Exception($"ValidateProgram failed: {infoLog}");
            }
#endif
            */
