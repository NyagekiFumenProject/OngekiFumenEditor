...
namespace OngekiFumenEditor.Modules.AudioAdjustWindow.ViewModels
{
    ...
    public class AudioAdjustWindowViewModel : WindowBase, IAudioAdjustWindow
    {
        ...
        {
            NotifyOfPropertyChange(() => CurrentEditorName);
            editorDocumentManager.OnActivateEditorChanged += (n, _) => NotifyOfPropertyChange(() => CurrentEditorName);
        ...
        public Task<(bool isSuccess, string msg)> OffsetAudioFile(string inputWavFilePath, string saveWavFilePath, TimeSpan offset)
        {
            return Task.Run(() =>
            {
                try
                {
                    var audio = AcbGeneratorFuck.Generator.LoadAsWavFile(inputWavFilePath);
                    var offseted = AcbGeneratorFuck.Generator.AdjustDuration(audio, offset.TotalSeconds);
                    using var fs = File.OpenWrite(saveWavFilePath);
                    offseted.SaveTo(fs);
                    return (true, default);
                }
                catch (Exception e)
