...
- **PERF-FWK-004 / FWK-REQUERY-STUCK-001 — P2，异常会永久卡住 requery。**〔**2026-09-21 已修复**，见下方「已修复项」〕`G/src/.../Framework/Commands/CommandManager.cs:67-97` 设置 broadcast flag 后直接调用 subscribers，`:96` reset 无 finally；异常会让后续非强制 invalidation 在 `:70` 永久 return。finally reset 并隔离 subscriber failure。
...
> 本分区条目体例与第 4、6、7 节一致：根因 → 处理 → 行为差异 → 验证 → 未做/后续。

- **PERF-FWK-004 / FWK-REQUERY-STUCK-001 — 已修复（2026-09-21）：异常不再永久停摆 requery，pass 期间的订阅变更不再中断 pass。**
  - **根因（静态确认 + 回归复现）：** `G/src/Gekimini.Avalonia/Framework/Commands/CommandManager.cs`（修复前 `:66-96`）在 `RaiseRequerySuggestedImpl` 里直接 `foreach` 调用所有 subscriber，`isBroadcastEvent` 只在正常路径结尾 `:96` 复位：①任一 subscriber 抛异常即跳过复位，之后所有非强制 invalidation 都在入口 `:69-70` 被永久丢弃。`CommandRouter.FindCommandHandlerInVisualTree:130` 存在可经 `TargetableCommand.CanExecute` 进入 requery 的 `InvalidOperationException`（data context 是 `ICommandRerouter` 且其 `GetHandler` 返回不支持该定义的 handler），所以不是纯理论路径；②订阅表是 `List<WeakReference<EventHandler>>`，pass 遍历期间发生订阅/退订（菜单重建会做）会抛「集合已修改」并同样卡死；③节流用 `DateTime.UtcNow` 墙钟，`async void` 会把异常抛到 dispatcher，并把输入事件的 `RoutedEventArgs` 保存到 100 ms 之后再交给 subscriber。
  - **处理：** 订阅表改为 copy-on-write 的 `WeakReference<EventHandler>[]`（读取端无锁：pass 取快照，pass 期间订阅/退订不影响本轮；死亡弱引用在 pass 结束后一次性重建，替代原 `HashSet` + 逐个 `List.Remove`）；每个 subscriber 单独 `try/catch` 并 `Trace`，失败不再中断其余订阅者；调度标志用 `Interlocked.CompareExchange` 独占、`finally` 复位（异常也无法让 scheduler 卡死）；节流改用 `Stopwatch` 单调时钟；删除无调用者的 `force` 形参；`RequerySuggested` 统一以 `null`/`EventArgs.Empty` 通知（pass 是状态重评估，与 WPF 一致）。
  - **行为差异：** 输入触发的 pass 不再把 `InputElement` 与该次输入的 `RoutedEventArgs` 交给 subscriber（此前也是延迟 ≥100 ms 才送达）；pass 期间到达的 invalidation 仍合并进当前 pass（既有语义，见未做/后续）。
  - **验证：** 新增 `tests/OngekiFumenEditor.Avalonia.Tests/UI/CommandManagerRequeryTests.cs` 两项行为契约——失败 subscriber 之后仍继续通知且后续 pass 仍运行、pass 期间订阅变更不中断 pass。修复前两项均失败（计数器停在 0：pass 在失败 subscriber 处中断），修复后通过。Release 全量 **806/806**。
  - **未做/后续：** ①pass 运行期间到达的 invalidation 仍被丢弃而非排入下一次 pass（担心自激励 handler 把 requery 变成永久高频循环；当前上界是 100 ms 一次）。若后续出现「命令状态要等下一次外部触发才刷新」的可见问题，再改为有界 follow-up pass。②`InvalidateRequerySuggested(reason)` 的 `reason` 仍未参与任何逻辑，仅作 API 兼容保留。③本分区 **PERF-FWK-003 / FWK-ROUTER-001**（每次 `CanExecute` 全量视觉树扫描）仍是 requery 的主要成本：`CommandManager` 的 100 ms 节流恰好是它的上界保护，后续收益应从 router 侧取。
...
- **2026-09-21 后续修复：** PERF-FWK-004 / FWK-REQUERY-STUCK-001（`G/src/Gekimini.Avalonia/Framework/Commands/CommandManager.cs`：订阅表 copy-on-write + 每个 subscriber 异常隔离 + `Stopwatch` 单调时钟节流 + 调度标志 `finally` 复位 + 删除无调用者的 `force` 形参；新增 2 项行为回归 `tests/OngekiFumenEditor.Avalonia.Tests/UI/CommandManagerRequeryTests.cs`，修复前失败、修复后通过；Release 全量 **806/806**）。其余发现状态不变；requery 的主要成本仍在 PERF-FWK-003 / FWK-ROUTER-001。
