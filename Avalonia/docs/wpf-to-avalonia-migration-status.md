...
Desktop 测试项目（**TFM 与主测试项目不同**，输出目录是 `net11.0-windows10.0.19041.0`，用 `net11.0` 会报 `No such file or directory`）：

```powershell
dotnet build .\tests\OngekiFumenEditor.Avalonia.Desktop.Tests\OngekiFumenEditor.Avalonia.Desktop.Tests.csproj -c Debug
.\tests\OngekiFumenEditor.Avalonia.Desktop.Tests\bin\Debug\net11.0-windows10.0.19041.0\OngekiFumenEditor.Avalonia.Desktop.Tests.exe
```

- 2026-09-12 实测：Release 全量 **698/698 通过**（含 `SkiaRenderSmokeTests` 像素断言、`DrawPlayableAreaHelperTests`、`ConnectableStartObjectChildRangeTests`、`DrawTimeSignatureHelperTests`、`DrawingFrameSnapshotTests`、`EditorUiRegressionTests` 等全部 headless 用例）；Desktop 测试项目 `-c Debug` 148/148 通过。
...
- 2026-09-13 性能：`PERF-RND-017 / RND-20`（replay engine 每帧创建）已修复——replay 改为按 render context 缓存（`DefaultSkiaDrawingManagerImpl.replayCache`），canvas 与帧状态从构造函数移出，改为每帧 `BeginFrame`（绑定 canvas + 清空 3 个矩阵栈 + 写入帧首状态）→ `Present` → `EndFrame`；`Dispose` 补全到全部 `IDisposable` backend 并幂等；`ReleaseRenderControl` 释放并移除缓存的 replay（释放接线由"每帧兜底"改为显式 teardown）。这同时让 `PERF-RND-011`/`RND-014` 的实例级缓存（path 池、dash effect 缓存、`SKFont`/`SKPaint`）第一次真正跨帧存活。新增基准 `benchmarks/.../ReplayLifecycleBenchmarks.cs`（64 帧合计 → 单帧）：**3.15 µs / 36,504 B → 14.0 ns / 0 B（225×，Gen0/Gen1 归零）**；新增 `tests/.../Graphics/ReplayLifecycleTests.cs` 5 项（4 项生命周期契约 + 1 项跨帧复用 headless 像素断言）。Release 全量 **703/703 通过**，Desktop 测试项目 148/148 通过。详见 `performance-gc-audit-2026-09-09.md` 的「已修复项」。
- 2026-09-14 修复：`PERF-DAT-009 / DAT-12` —— `BulletPalleteList` 的 `public BulletPallete this[int index] => this[index];` 无条件调用自身，任何按下标读取都以**不可捕获**的 `StackOverflowException` 结束进程，而该类型公开声明实现 `IReadOnlyList<BulletPallete>`。改为按 `ConvertIdToInt(StrID)` 升序维护的 backing 列表（`Count`/`this[int]`/`GetEnumerator` 共用，插入二分定位，顺序与旧 `OrderBy(ConvertIdToInt)` 逐项等价）；`RemovePallete` 的退订改作用于真正入表的实例。顺带消除旧 `GetEnumerator` 每次枚举都 `OrderBy` 重排 + 每元素 `ConvertIdToInt` 的成本——两个解析器对**每条** bullet/bell 命令都做一次全列表查找，是这条真实的热路径。新增基准 `benchmarks/.../BulletPalleteListAccessBenchmarks.cs`（Release / DefaultJob）：按下标走一遍 256 项 **5.64 ms / 11.06 MB → 209 ns / 0 B**；解析期逐命令查找（256 条命令）**6.00 ms / 11.10 MB → 163.7 µs / 30.7 KB**；枚举一次 256 项 22.7 µs / 43.3 KB → 222 ns / 0 B。新增 `tests/.../Base/Collections/BulletPalleteListTests.cs` 13 项。Release 全量 **716/716**、Desktop 测试项目 148/148 通过。WPF 侧同文件有同一处索引器自递归（`OngekiFumenEditor/Base/Collections/BulletPalleteList.cs:47`），未随本轮改动。详见 `performance-gc-audit-2026-09-09.md` 第 6 节「已修复项」。
- 2026-09-16 修复：`PERF-DAT-010 / DAT-13` —— `MeterChangeList` 的 `GetEnumerator()` 每次枚举都对**本就已升序**的 backing（`TGridSortList<MeterChange>`）重跑一遍 `OrderBy(x => x.TGrid)`；`GetMeter`/`GetPrevMeter`/`GetNextMeter` 则是 `this.LastOrDefault(...)`/`FirstOrDefault(...)`，因该类型不实现 `IList<T>` 而走 `Enumerable` 非列表路径，**每次调用都先完整枚举一遍（含重排）再线性扫描**。改法：枚举直接走 backing（去掉 `OrderBy`）；三个查询改用 backing 的 lower/upper bound 二分（`GetMeter` = `UpperBoundIndex - 1`、`GetPrevMeter` = `LowerBoundIndex - 1`、`GetNextMeter` = `UpperBoundIndex`，`firstMeter` 退化分支单独判断）。为此在 `SortableCollection<T,X>` **新增** `LowerBoundIndex`/`UpperBoundIndex`（纯新增、不改既有语义，且没有 `BinarySearchBy` 的等值段线性尾部——顺带为 `PERF-DAT-002 / DAT-02` 备好基建）。**行为与旧实现逐项等价**（含 `firstMeter` 不是列表最小值这一既有边界）；唯一差异是枚举器由快照变 live，边枚举边改会抛 `InvalidOperationException`（与 DAT-009 同款，已核对 caller 均只读，三个变更入口不在枚举体内）。真实热点有二：`GetAllDisplayableObjects()` 里 `.Concat(MeterChanges.Skip(1))` 把重排放大到几乎每个交互/检查/格式化路径；`DefaultFumenSoundPlayer.CalculateHoldTicks` **逐 Hold** 调 `GetMeter`。新增基准 `benchmarks/.../MeterChangeListQueryBenchmarks.cs`（Release / DefaultJob，旧侧按 09-09 基线逐字 LINQ 复刻、新侧调真实类型）：逐 Hold `GetMeter`×256 **1,986.138 µs / 1,419,264 B → 4.581 µs / 0 B**；早退 `.Skip(1).FirstOrDefault()` 256 档 **6,253.41 ns / 5,520 B → 39.64 ns / 144 B**；枚举一次 256 档 **7,187.71 ns / 5,464 B → 1,376.79 ns / 88 B**；逐点 `GetPrev`+`GetNext` 256 档 3.75 ms / 2.83 MB → 10.21 µs / 0 B。新增 `tests/.../Base/Collections/MeterChangeListTests.cs`（含与逐字复刻旧实现的对拍）与 `SortableCollectionBoundsTests.cs`，共 36 项。Release 全量 **752/752**、Desktop 测试项目 148/148 通过。`GetPrevMeter`/`GetNextMeter` 在 Avalonia/src 内无 caller（潜在 API）；WPF 侧同文件有同一处缺陷（`OngekiFumenEditor/Base/Collections/MeterChangeList.cs:72-89`），未随本轮改动。详见 `performance-gc-audit-2026-09-09.md` 第 6 节「已修复项」。
