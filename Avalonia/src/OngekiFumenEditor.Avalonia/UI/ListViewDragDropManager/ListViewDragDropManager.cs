...
namespace OngekiFumenEditor.Avalonia.UI.ListViewDragDropManager
{
    ...
    public class ListViewDragDropManager<ItemType> where ItemType : class
    {
        ...
        void listView_Drop(object sender, DragEventArgs e)
        {
            ...
            ItemType data = e.Data.GetData(e.Data.GetFormats()[0]) as ItemType;
            if (data == null)
                return;

            // Get the ObservableCollection<ItemType> which contains the dropped data object.
            IList<ItemType> itemsSource = this.listView.ItemsSource as IList<ItemType>;
            if (itemsSource == null)
                throw new Exception(
                    $"A ListView managed by {nameof(ListViewDragDropManager<ItemType>)} must have its ItemsSource set to an IList<{typeof(ItemType).Name}>.");
            ...
            else
            {
                ...
                if (oldIndex > -1)
                {
                    if (itemsSource is ObservableCollection<ItemType> observableCollection)
                        observableCollection.Move(oldIndex, newIndex);
                    else
                        throw new Exception(
                            $"A ListView managed by {nameof(ListViewDragDropManager<ItemType>)} must have its ItemsSource set to an ObservableCollection<{typeof(ItemType).Name}>.");
                }
                else
        ...
        bool CanStartDragOperation
        {
            get
            {
                ...
                if (this.listView?.SelectedItem is not ItemType)
                    return false;
