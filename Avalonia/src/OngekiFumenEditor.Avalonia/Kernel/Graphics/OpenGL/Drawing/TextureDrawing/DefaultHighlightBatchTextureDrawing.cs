...
namespace OngekiFumenEditor.Avalonia.Kernel.Graphics.OpenGL.Drawing.TextureDrawing
{
    internal class DefaultHighlightBatchTextureDrawing : CommonOpenGLDrawingBase, IHighlightBatchTextureDrawing, IDisposable
    {
        ...
        */
        ...
        public DefaultHighlightBatchTextureDrawing(DefaultOpenGLRenderManagerImpl manager) : base(manager)
        {
        ...
        public void Draw(IDrawingContext target, IImage texture, IEnumerable<(Vector2 size, Vector2 position, float rotation, Vector4 color)> instances)
        {
            ...
            foreach ((Vector2 size, Vector2 position, float rotation, _) in instances)
                PostSprite(size, position, rotation, default);
            FlushDraw();
            End();
        }
