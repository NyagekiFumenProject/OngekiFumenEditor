				...
				for (int i = 0; i < walls.Count - 1; i++)
				{
					...
					if (cur.TGridRange.IsInRange(next.TGridRange, false) || next.TGridRange.IsInRange(cur.TGridRange, false))
					{
						yield return new CommonCheckResult()
						{
							...
							Description = Lang.WallConflict.Format(cur.Wall.RecordId, next.Wall.RecordId),
					...
					foreach (var child in wall.Wall.Children)
					{
						if (child.TGrid < maxTGrid)
						{
							yield return new CommonCheckResult()
							{
								...
								Description = Lang.WallConflict3.Format(wall.Wall.RecordId, child.TGrid, maxTGrid),
					...
					if (rightWall.XGridRange.IsInRange(leftWall.XGridRange, false))
					{
						...
						if (point is Point p)
						{
							...
							yield return new CommonCheckResult()
							{
								...
								Description = Lang.WallConflict.Format(leftWall.Wall.RecordId, rightWall.Wall.RecordId),
