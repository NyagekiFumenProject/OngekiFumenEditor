    ...
    public class DrawPlayableAreaHelper : IDisposable
    {
        ...
        private sealed class FieldAreaFrameContext : IDisposable
        {
            ...
            public FieldAreaFrameContext(IFumenEditorDrawingContext target, SoflanList soflanGroup, TGrid minTGrid, TGrid maxTGrid)
            {
                ...
                foreach (var lane in target.Editor.EditorContext.Fumen.Lanes.GetVisibleStartObjects(minTGrid, maxTGrid))
                {
        ...
        private static void CollectBaseSampleTGrids(FieldAreaFrameContext context, ISet<int> result)
        {
            ...
            AddSoflanSamples(
                context.SoflanGroup.GetCachedSoflanPositionList_PreviewMode(context.Target.Editor.EditorContext.Fumen.BpmList),
                minTotalTGrid,
                maxTotalTGrid,
                result,
                ref prevContextTotalTGrid,
