...
namespace OngekiFumenEditor.Avalonia.Modules.FumenVisualEditor.Graphics.Drawing.TargetImpl.EditorObjects
{
    ...
    public class LaneCurvePathControlDrawingTarget : CommonBatchDrawTargetBase<LaneCurvePathControlObject>, IDisposable
    {
        ...
        public override void DrawBatch(IFumenEditorDrawingContext target, IDrawCommandListBuilder builder, IEnumerable<LaneCurvePathControlObject> objs)
        {
            ...
            using var allTex = ObjectPool.GetPooledList<(Vector2 size, Vector2 position, float rotation, Vector4 color)>();
            using var selectedTex = ObjectPool.GetPooledList<(Vector2 size, Vector2 position, float rotation, Vector4 color)>();
            ...
            foreach (var obj in objs)
            {
                ...
                filtered.Add(point);
                allTex.Add((size, new Vector2(x, y), 0f, Vector4.One));
                if (obj.IsSelected)
                    selectedTex.Add((size * 1.25f, new Vector2(x, y), 0f, Vector4.One));
            ...
            for (var i = 0; i < filtered.Count; i++)
            {
                ...
                builder.DrawString(p.Obj.Index.ToString(), new(p.X, p.Y + 4), Vector2.One, 15, 0, new(1, 0, 1, 1), new(0.5f, 0.5f),
