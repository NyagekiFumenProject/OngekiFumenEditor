...
namespace OngekiFumenEditor.Avalonia.Utils
{
    public static class ResourceUtils
    {
        ...
        public static Stream OpenReadFromLocalAssemblyEmbbedResources(string resourceName)
            => typeof(ResourceUtils).Assembly.GetManifestResourceStream("OngekiFumenEditor.Avalonia.Lang." + resourceName);

        public static Stream OpenReadResourceStream(string relativeUrl)
        {
            if (relativeUrl.StartsWith("avares://", StringComparison.OrdinalIgnoreCase))
                return AssetLoader.Open(new Uri(relativeUrl, UriKind.Absolute));

            var localPath = relativeUrl.TrimStart('.', '\\', '/').Replace('/', Path.DirectorySeparatorChar);
            if (File.Exists(localPath))
                return File.OpenRead(localPath);

            throw new FileNotFoundException($"Resource not found: {relativeUrl}");
        }
