...
using PoolFacade = OngekiFumenEditor.Avalonia.Utils.ObjectPool.ObjectPool;
...
namespace OngekiFumenEditor.Avalonia.Utils
{
    public static class LinqExtensionMethod
    {
        ...
        {
            using var removes = collection.Except(collection.DistinctBy(keySelect)).ToListWithObjectPool();
            foreach (var rm in removes)
                collection.Remove(rm);
        }
